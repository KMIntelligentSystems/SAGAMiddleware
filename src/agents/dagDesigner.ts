...
export class DAGDesigner extends BaseSDKAgent {
    ...
    private async handleOutputDAG(args: any) {
        try {
            ...
            fs.writeFileSync(
                'C:/repos/SAGAMiddleware/data/DagDesignerOut_tool_doc.json',
    ...
    async execute(input: DAGDesignerInput): Promise<AgentResult> {
        try {
            ...
            console.log('🤔 Analyzing requirements and formulating DAG plan...');
             ...
             const output =  JSON.parse(fs.readFileSync('C:/repos/SAGAMiddleware/data/DagDesignerOut_tool_doc.json', 'utf-8')) as DAGDefinition;//await this.executeQuery(prompt);     // JSON.parse(fs.readFileSync('C:/repos/SAGAMiddleware/data/DagDesignerOut_tool.json', 'utf-8')) as DAGDefinition;//
