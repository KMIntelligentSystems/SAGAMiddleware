...
export interface D3VisualizationOptions {
  ...
  csvData?: string;        // CSV data content to intercept and provide to d3.csv() calls
  csvFilename?: string;    // CSV filename pattern to intercept (e.g., 'data.csv')
...
export class D3VisualizationClient {
  ...
  async renderD3(options: D3VisualizationOptions): Promise<D3RenderResult> {
    try {
      console.log('🎨 Starting D3 visualization rendering...');
      console.log(`🔍 Checking connection to server: ${this.serverName}`);
      ...
      if (!this.mcpClient.isConnected(this.serverName)) {
        console.error(`❌ Server '${this.serverName}' is not connected`);
        console.error(`💡 Available servers: ${connectedServers.join(', ')}`);
        console.error(`💡 Make sure Playwright MCP server is connected during initialization`);
      ...
      if (options.data) {
        console.log('💾 Injecting additional data...');
        await this.mcpClient.callTool(this.serverName, {
