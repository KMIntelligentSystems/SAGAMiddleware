  ...
  private registerDefaultContextSet(): void {
    ...
    const defaultLLMPrompts: LLMPromptConfig[] = [
      ...
      },
     {
        ...
        backstory: 'Provide files for indexing using tool calls.',
        taskDescription: `You are a tool calling agent. You must call a MCP server with this arg: { "jsonrpc": "2.0", "id": 3, "method": "tools/call", "params": { "name": "execute_python", "arguments": { "code": {code}} } } Where the tool call is to "execute_python". 
        Note {code} in above. This is the placeholder for the code generated by the coding agent and the generated code must be placed here. `,
...
if(opType === 'profile_approved'){
   ...
   */
  private async executeThreadVisualizationSAGA(browserRequest: BrowserGraphRequest, threadId: string, profiledPrompt: string): Promise<void> {
    try {
