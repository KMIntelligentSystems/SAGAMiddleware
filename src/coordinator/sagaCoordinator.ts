...
import { DataResultProcessor, globalDataProcessor, ProcessedDataWithKey } from '../processing/dataResultProcessor.js';
...
if(definition.agentType === 'tool'){
  ...
  private hasCycleDependencyFromFlow(transaction: SagaTransaction): boolean {
    // Check if this transaction is part of a cycle in any agent flow
    for (const flow of this.agentFlows) {
      const firstIndex = flow.indexOf(transaction.id);
      if (firstIndex === -1) continue;
      
      // Check if the same agent ID appears again later in the flow (indicating a cycle)
      const lastIndex = flow.lastIndexOf(transaction.id);
      if (lastIndex > firstIndex) {
        console.log(`🔗 Cycle detected for ${transaction.id} in flow: first at index ${firstIndex}, last at ${lastIndex}`);
        return true;
      }
    }
...
graphAnalyzerResult 
    ...
    '- Columns: date/time,installation,energy_source,MW\n' +
         ...
         dynamicTransactionSet = collection;
          ...
          lastExecutionResult = executionResult; // Update the last execution result for next iteration
...
//  Processing dynamic set: Data Processing Set (processing-set)agent-generating-set
          ...
          if (setId === 'processing-set') {
            // Processing set logic will be implemented here
          if(contextSet){
             contextSet = {
                name: 'processing',
                 transactionSetName: 'ProcessingSet',
                  dataSources: [],
                  llmPrompts: [],
                  userQuery: ''
           }
          ...
          if (setId === 'saving-set') {
             ...
             contextSet = {
                name: 'saving',
                 transactionSetName: 'SavingSet',
                  dataSources: [],
                  llmPrompts: [],
                  userQuery: lastExecutionResult?.result
           }
          }
...
//console.log('DYNAMIC 2', dynamicSetResult.success)
            ...
            const dynamicExecutionResult: SetExecutionResult = {
              ...
              metadata: {
                ...
                transactionsExecuted: dynamicSet.transactions.length,
                transactionsFailed: dynamicSetResult.success ? 0 : 1
              }
            };

            if (dynamicSetResult.error) {
  ...
  async executeSagaWorkflow(
    ...
    contextSet?: ContextSetDefinition,
    lastExecutionResult?: SetExecutionResult
  ): Promise<AgentResult> {
    console.log(`🚀 SC: ${workflowId}`);
    
    // Handle default values for first iteration when lastExecutionResult is undefined
    const isFirstIteration = !lastExecutionResult;
    
    if (isFirstIteration) {
      console.log('📝 First iteration - using default values for execution context');
    } else {
      console.log(`📝 Continuing iteration - previous result from set: ${lastExecutionResult.setId}`);
    }
 ...
 return {
  ...
  private async executeSagaTransaction(
    ...
    //Starting point for call to new class. Use conversationContext.lastActionResult to get user request
  ...
  private async buildSagaTransactionContext(
    ...
    const contextSet: ContextSetDefinition = this.contextManager.getActiveContextSet();
   ...
   const transactionSetId = transaction.transactionSetId;
    ...
    if (currContextSet.dataSources && currContextSet.dataSources.length > 0) {
     ...
     console.log(`TRANSACTION AGENT NAME`, transaction.dependencies.map(t => `${t}`));
    ...
    const currTransactionSet = contextSet.transactionSetName;
    let agentSpecificTask = 'AS ABOVE: TASK DESCRIPTION';
    ...
    //  console.log('AGENT ',this.agents.get('TransactionGroupingAgent'))
      ...
      agentSpecificTask = request.userQuery || 'AS ABOVE: TASK DESCRIPTION';
    } 
 ...
 //   const agent = this.agents.get(iteratingTransaction.agentName);
   ...
   /* while(iteration < 2){
      if(iteration === 1){
        ...
        result = await this.executeSagaTransaction(iteratingTransaction, request);
       const agentDefn: AgentDefinition = agent?.getAgentDefinition() as AgentDefinition;
       agentDefn.taskDescription = 'Your task is to follow instuctions as provided below under YOUR TASK:';
       agentDefn.taskExpectedOutput= 'The output as requested is provided in <flow></flow>';
       agent?.setAgentDefinition(agentDefn);
        agent?.deleteContext();
  ...
  //    console.log('ITERATION ZERO ', result.result)
        ...
        this.contextManager.updateContext(iteratingTransaction.agentName, {
        lastActionResult: groupCtx.previousResult,
        transactionId: iteratingTransaction.id,
        timestamp: new Date()
      });
...
//const lastResult = JSON.stringify(groupedAgentContext.lastTransactionResult);
    ...
    //DataRetrievalAgent  DateNormalizationAgent  PerPageAggregatorAgent
    console.log('TRANSACTION  ', transaction.agentName)
        ...
        const cycleContext = await this.buildCycleTransactionContext(
          ...
          if ( i === 0) {
              const agent = this.agents.get(transaction.agentName);
              agent?.getAgentDefinition().taskDescription.replace(/{page}/g, cycleContext?.page);
              agent?.receiveContext({'If {page} occurs in the query, then replace it with this number: ': cycleContext?.page})
             // cycleContext.agentSpecificTask = /*'Replace {page} in query with this value: ' + */page.toString();//()cycleContext.agentSpecificTask.replace(/{page}/g, page.toString());
               page++;
   ...
   //       console.log('RESULT  FILTERINGDATA', result);
          ...
          //  console.log('CLEAN JSON PREVIEW:', cleanJson);
            ...
            try {
              ...
              if (parsedResult.results && Array.isArray(parsedResult.results) && parsedResult.results.length > 0) {
                ...
                //Server just returns page number sent to it
              //  const currentPage = parseInt(parsedResult.page) || 1;
                const totalPages = parseInt(parsedResult.total_pages) || 1;
                
               
                hasMoreChunks = page < totalPages;
                
                console.log(`📊 DataFilteringAgent pagination: page=${page}, total_pages=${totalPages}, resultsReturned=${parsedResult.results.length}, hasMoreChunks=${hasMoreChunks}`);
              } else {
                hasMoreChunks = false;
        ...
        if (nextAgent && i < cycleTransactions.length - 1) {
              ...
              console.log('PASSED RESULT ', finalResult.result)
...
//END
  ...
  /**
   * Legacy method for specific extended cycle loop (moved for compatibility)
   */
  private async executeLegacyExtendedCycleLoop(
    cyclePartners: SagaTransaction[], 
    request: BrowserGraphRequest
  ): Promise<AgentResult> {
    const groupedContext = this.contextManager.getContext('TransactionGroupingAgent') as WorkingMemory;
    ...
    for (let i = 0; i < cyclePartners.length; i++) {
      ...
      const agentCtx = this.contextManager.getContext('TransactionGroupingAgent') as WorkingMemory;
      previousRes = JSON.stringify(agentCtx.previousResult);
      this.contextManager.updateContext(cyclePartners[i].agentName, {
        lastTransactionResult: previousRes,
        transactionId: cyclePartners[i].id,
        timestamp: new Date(),
        processedInCycle: true
      });
    }
    ...
    this.contextManager.updateContext('TransactionGroupingAgent', {
      lastTransactionResult: previousRes,
      transactionId: 'TransactionGroupingAgent',
      timestamp: new Date(),
      processedInCycle: true
    });
